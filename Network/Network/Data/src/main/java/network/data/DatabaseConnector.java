...
public class DatabaseConnector {
    ...
    public void signUp(SignUpData rawData) {
    	Connection connection = null;
        Statement statement = null;
        try{
            // 注册 JDBC 驱动
            ...
            statement = connection.createStatement();
            String sql;
            sql = String.format("INSERT INTO users ( userName, password ) VALUES (\'%s\', \'%s\');",
            		rawData.getUserName(),
            		rawData.getPassword());
	...
	public boolean existCheck(SignUpData rawData) {
		Connection connection = null;
        Statement statement = null;
        try{
            // 注册 JDBC 驱动
            ...
            statement = connection.createStatement();
            String sql;
            sql = String.format("SELECT userName, password FROM users WHERE userName= \'%s\'",
            		rawData.getUserName());
            ResultSet rs = statement.executeQuery(sql);
